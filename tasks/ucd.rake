...
namespace :taxonworks do
  namespace :ucd do
    ...
    # This read-only task takes data sensu Yanega's list and reports proposed changes mismatched names on either side
    #
    # rake taxonworks:ucd:inspect_genus_group_name_gender gender_data=/Users/matt/src/github/chalcid/tw_curation/tasks/data/GenderedChalcidoidea24vi25.tsv
    desc 'inspect genus group name gender'
    task inspect_genus_group_name_gender: [ :curation] do
      ...
      g = CSV.read(ENV['gender_data'], headers: true, col_sep: "\t")
      ...
      g.each do |row|
        ...
        n = row['genericName']
        inspected_names.push n

        gender = row['gender'].split(/\s/).first

        if !mapped_keys.keys.include?(gender)
          bad_source_data.push "#{n}: #{gender}"
          next
        end
        ...
        case q.size
        when 0
          missing_in_taxonworks.push n
        when 1
          t = q.all.first

          r.push t.id
          r.push t.cached_gender
          r.push mapped_keys[gender]

          if t.cached_gender.present?
            r.push t.cached_gender == mapped_keys[gender] ? nil : true # there is a disagreement
          else
            r.push nil
          end

        else
          r.push q.all.pluck(:id).join(';')
          r.push q.all.pluck(:cached_gender).join(';')
          r.push mapped_keys[gender]
          r.push 'not checked'
        end
      ...
      # The proposed gender table
      # puts %w{genericName taxon_name_id current_gender proposed_gender disagreement}.join(',')
      # puts d.collect{|r| r.join(',')}.join("\n")

      # Genera missing in TaxonWorks
      # puts missing_in_taxonworks.join("\n")

      # Badly formatted source data
      puts bad_source_data.join("\n")

      x = ::Queries::TaxonName::Filter.new(
        descendants: true,
        nomenclature_group: 'Genus',
        taxon_name_id: 455458,
        project_id:
      ).all.pluck(:cached)

      # Names in UCD not in Yanega list
      # puts (x - inspected_names).sort.join("\n")
    end


    # This task takes a report sensu the task above and assigns
    # the respective values to genus group names ONLY IF the record is blank,
    # mismatched asignments are logged and skipped.
    desc 'batch update genus group gender'
    task batch_update_genus_group_gender: [:curation] do
    end

    # UNUSED, remove
    # rake taxonworks:ucd:add_species_asserted_distributions map_file=/Users/matt/src/github/chalcid/tw_curation/tasks/data/geographic_area_id_mappings.tsv
    desc 'add SPECIES AssertedDistributions'
    task add_species_asserted_distributions: [ :curation ] do

      predicate_id = 349 # Species::Country
      project_id = Current.project_id

      raise 'no map_file=/../' if ENV['map_file'].nil?
      g = CSV.read(ENV['map_file'], headers: true, col_sep: "\t")

      map_data = g.collect{|r| [r.fetch('Code'), r.fetch('GeographicAreaID')] }.to_h

      attribute_query = InternalAttribute.where(
        controlled_vocabulary_term_id: 349, #  'Species::Country'
        attribute_subject_type: 'TaxonName',
        project_id:
      )

      data = attribute_query.pluck(:id, :attribute_subject_id, :value)
      ...
      AssertedDistribution.transaction do |t|
        TaxonName
          .with(attribute_join: attribute_query)
          .joins('JOIN attribute_join aj on aj.attribute_subject_id = taxon_names.id')
          .joins(:source, :otus)
          .where(
            otus: {name: nil})
          .select('taxon_names.id, sources.id  source_id, value')
          .distinct
          .find_each do |n|

            otu_id = nil
            otu_ids = n.otus.where(name: nil).pluck(:id)
            source_id = n.source_id

            geographic_area_id = map_data[n.value]

            # TODO: Log
            next if geographic_area_id.nil?

            if otu_ids.size > 1
              otu_id = otu_ids.first
              # puts n.otus.where(otus: {name: nil}).pluck(:id).join(',')
            elsif otu_ids.size == 0
              o = Otu.create!(
                taxon_name_id: n.id,
                project_id:
              )
            ...
            begin
              puts [otu_id, geographic_area_id, source_id].join(', ')
              # In a pre-trial run we found zero overlap, so we can simplly create away
              AssertedDistribution.create!(
                otu_id:,
                asserted_distribution_shape_id: geographic_area_id,
